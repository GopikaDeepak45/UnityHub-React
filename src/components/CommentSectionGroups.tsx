...
import { useAddCommentGroupsMutation, useFetchCommentsGroupsQuery } from "@/redux/apiSlices/groupApiSlice";
import CommentLikeButtonGroups from "./CommentLikesButtonGroups";
...
type FormData = z.infer<typeof formSchema>;
  const CommentSection: React.FC<CommentSectionProps> = ({ postId,groupId }) => {
     const{userId}=useAuth()
  const{data:comments,isLoading,error}=useFetchCommentsGroupsQuery({postId,userId})

  const [addCommentGroups]=useAddCommentGroupsMutation()
  ...
  if (error) return <ErrorComponent message={error.data.message} />;
    return (
      ...
      <div className="p-5  border rounded-md m-2 ">
        ...
        <CommentLikeButtonGroups commentId={comment._id} likes={comment.likes.length} likesArray={comment.likes}/>
          </div>
          </div>
        ))}
        </div>
